...
def display_links(tab_name):
    ...
    for i, link in enumerate(paged_links):
        ...
        if col1.button(link["description"], key=f"link_{user_id}_{tab_name}_{idx}"):
            open_link(link["url"])
...
st.markdown(
    """
    <h1 style='color: deep orange;'>E1 Link</h1>
    """,
    unsafe_allow_html=True,
)

if search_query and search_query.strip():
    # 검색어가 있을 때, 모든 탭의 모든 링크 중 description에 포함된 것만 리스트업
